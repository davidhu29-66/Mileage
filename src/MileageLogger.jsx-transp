...
export default function MileageLogger() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col relative" style={{ fontFamily: "'Manrope', sans-serif" }}>
      ...
      {/* Persistent Clean Wallpaper */}
      {appBgImage && (
        <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none">
          <img src={appBgImage} alt="" className="w-full h-full object-cover object-center" />
          <div className="absolute inset-0 bg-gradient-to-b from-slate-950/70 via-slate-950/40 to-slate-950/75" />
        </div>
      )}
...
function Header({ lastMileage, lastMileageMeta, activeTrip }) {
  return (
    <div className="px-5 pt-6 pb-5 border-b border-slate-800/80 bg-slate-950/80">
      <div className="flex items-center justify-between mb-3">
        ...
        {activeTrip && (
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-950 border border-emerald-500/40">
...
function LogTab({ activeTrip, recentTrips, onStart, onEnd, onFull, onViewAll, onEditTrip }) {
  return (
    <div className="space-y-4">
      {activeTrip ? (
        <div className="rounded-2xl bg-slate-900/90 border border-emerald-500/40 p-4">
      ...
      <button
        onClick={onFull}
        className="w-full py-3 rounded-xl bg-slate-900/90 border border-slate-700/80 text-slate-300 font-semibold text-sm flex items-center justify-center gap-2 active:scale-95 transition-transform"
      >
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <button onClick={() => setMonthOffset((m) => m - 1)} className="w-9 h-9 rounded-full bg-slate-900/90 border border-slate-700 flex items-center justify-center active:scale-95">
        ...
        <button
          ...
          className="w-9 h-9 rounded-full bg-slate-900/90 border border-slate-700 flex items-center justify-center active:scale-95 disabled:opacity-30"
        >
      ...
      {unspecifiedCount > 0 && (
        <div className="text-xs text-amber-300 bg-slate-900/90 border border-amber-400/40 rounded-xl px-3 py-2">
      ...
      <div className="rounded-2xl bg-slate-900/90 border border-slate-800 p-4 space-y-2">
        ...
        <button onClick={() => exportCsv("chargeable")} className="w-full py-2.5 rounded-xl bg-slate-800 border border-sky-400/30 text-sky-400 text-sm font-medium flex items-center justify-center gap-2 active:scale-95">
          <Receipt size={14} /> Chargeable only, for client billing (CSV)
        </button>
        <button onClick={exportSiteVisitsCsv} className="w-full py-2.5 rounded-xl bg-slate-800 border border-amber-400/30 text-amber-400 text-sm font-medium flex items-center justify-center gap-2 active:scale-95">
      ...
      <div className="rounded-2xl bg-slate-900/90 border border-slate-800 p-4">
        ...
        <div className="flex flex-col gap-2 mb-3">
          {locations.map((loc) => {
            ...
            return (
              <div key={loc.name} className="flex items-center gap-2 px-3 py-2 rounded-xl bg-slate-800 border border-slate-700">
                ...
                <button
                  ...
                  className={`text-xs font-medium px-2 py-1 rounded-lg ${hasCoords ? "text-slate-300 bg-slate-700" : "text-amber-400 bg-slate-900 border border-amber-400/30"}`}
                >
      ...
      <div className="rounded-2xl bg-slate-900/90 border border-slate-800 p-4">
        ...
        <div className="grid grid-cols-2 gap-2 mt-1">
          <button
            ...
            className="py-2.5 rounded-xl bg-slate-800 border border-slate-700 text-slate-200 text-sm font-medium flex items-center justify-center gap-2 active:scale-95 disabled:opacity-40"
          >
            <Send size={13} />
          ...
          <button
            ...
            className="py-2.5 rounded-xl bg-slate-800 border border-sky-400/30 text-sky-400 text-sm font-medium flex items-center justify-center gap-2 active:scale-95 disabled:opacity-40"
          >
            <Radio size={13} />
      ...
      <div className="rounded-2xl bg-slate-900/90 border border-slate-800 p-4">
        ...
        {typeof window !== "undefined" && window.appSignOut && (
          <button
            ...
            className="w-full py-2.5 rounded-xl bg-slate-800 border border-slate-700 text-slate-300 text-sm font-medium"
          >
            Sign out
          </button>
        )}
...
function BottomNav({ tab, setTab }) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-slate-950/90 border-t border-slate-800/80 px-2 pb-safe z-20">
...
function Modal({ title, onClose, children, footer, bgImage }) {
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div
        className="w-full max-w-md rounded-t-3xl border-t border-slate-700 relative overflow-hidden bg-slate-900"
        ...
        <div className="mt-2.5 pl-3 border-l-2 border-slate-800">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`flex-1 py-2 rounded-lg border text-xs font-semibold ${businessType !== "chargeable" ? "bg-slate-700 border-slate-500 text-slate-100" : "bg-slate-800 border-slate-700 text-slate-400"}`}
            >
              Admin
            </button>
            <button
              onClick={() => onBusinessTypeChange("chargeable")}
              className={`flex-1 py-2 rounded-lg border text-xs font-semibold flex items-center justify-center gap-1 ${businessType === "chargeable" ? "bg-sky-400/15 border-sky-400 text-sky-400" : "bg-slate-800 border-slate-700 text-slate-400"}`}
            >
...
function EndTripModal({ trip, locations, onClose, onSave }) {
  ...
  return (
    ...
    >
      <div className="rounded-xl bg-slate-800/80 p-3 mb-3 text-sm border border-slate-700/80">

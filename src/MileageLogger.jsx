...
export default function MileageLogger() {
  ...
  async function persistLocations(next) {
    ...
    try {
      ...
      showToast("error", "Couldn't save location.");
    }
  }
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <div className="text-slate-500 text-sm font-medium tracking-wide">Loading logbook…</div>
      </div>
    );
  }
  ...
  return (
    <div
      className="min-h-screen text-slate-100 flex flex-col relative bg-slate-950 bg-cover bg-center bg-no-repeat bg-fixed"
      style={{
        fontFamily: "'Manrope', sans-serif",
        backgroundImage: appBgImage ? `linear-gradient(rgba(15, 23, 42, 0.65), rgba(15, 23, 42, 0.65)), url(${appBgImage})` : "none",
      }}
    >
      <style>{`
...
function Header({ lastMileage, lastMileageMeta, activeTrip }) {
  return (
    <div className="px-5 pt-6 pb-5 border-b border-slate-800/60 bg-slate-950/40 backdrop-blur-sm">
      <div className="flex items-center justify-between mb-3">
        <div className="flex flex-col gap-1">
          <img src="/logo.png" alt="Company logo" className="h-8 w-auto object-contain object-left" />
          <span className="text-xs uppercase tracking-widest text-slate-400">Mileage Logbook</span>
        </div>
        {activeTrip && (
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-slate-900/90 border border-emerald-500">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
            <span className="text-xs font-medium text-emerald-400">Trip in progress</span>
          </div>
        )}
      </div>
      <div className="text-xs uppercase tracking-widest text-slate-400 mb-1">Last recorded odometer</div>
      <div className="flex items-baseline gap-2">
        <span className="font-odo text-4xl font-bold text-slate-50">{lastMileage !== null ? fmtKm(lastMileage) : "—"}</span>
        <span className="text-slate-400 text-sm font-medium">km</span>
      </div>
      {lastMileageMeta && (
...
function LogTab({ activeTrip, recentTrips, onStart, onEnd, onFull, onViewAll, onEditTrip }) {
  return (
    <div className="space-y-4">
      {activeTrip ? (
        <div className="rounded-2xl bg-slate-900/90 border border-emerald-500 p-4 shadow-lg backdrop-blur-md">
          ...
          <button
            onClick={onEnd}
            className="w-full py-3.5 rounded-xl bg-amber-400 text-slate-950 font-bold text-sm flex items-center justify-center gap-2 active:scale-95 transition-transform shadow-md"
          >
        ...
        <button
          onClick={onStart}
          className="w-full py-5 rounded-2xl bg-amber-400 text-slate-950 font-bold text-base flex items-center justify-center gap-2 active:scale-95 transition-transform shadow-lg"
        >
      ...
      <button
        onClick={onFull}
        className="w-full py-3 rounded-xl bg-slate-900/80 border border-slate-700/60 text-slate-200 font-semibold text-sm flex items-center justify-center gap-2 active:scale-95 transition-transform shadow-md"
      >
        <Plus size={16} /> Log a completed trip
      </button>

      <div className="rounded-2xl bg-slate-900/90 border border-slate-800/80 p-4 shadow-xl backdrop-blur-md">
        <div className="flex items-center justify-between mb-3">
...
function TripRow({ trip, onClick, compact }) {
  ...
  const iconWrapCls = isChargeable ? "bg-sky-950/80 text-sky-400" : isBiz ? "bg-emerald-950/80 text-emerald-400" : "bg-rose-950/80 text-rose-400";
  return (
    <button
      onClick={onClick}
      className="w-full text-left rounded-xl bg-slate-900/95 border border-slate-800/90 p-3 flex items-center gap-3 transition-colors active:bg-slate-800"
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-xs text-slate-400">
          ...
          {isBiz && trip.businessType && (
            <span className={isChargeable ? "text-sky-400" : "text-slate-400"}>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <button onClick={() => setMonthOffset((m) => m - 1)} className="w-9 h-9 rounded-full bg-slate-900/90 border border-slate-800 flex items-center justify-center active:scale-95">
        ...
        <button
          ...
          className="w-9 h-9 rounded-full bg-slate-900/90 border border-slate-800 flex items-center justify-center active:scale-95 disabled:opacity-30"
        >
      ...
      {unspecifiedCount > 0 && (
        <div className="text-xs text-amber-400 bg-slate-900/90 border border-amber-500/30 rounded-xl px-3 py-2">
      ...
      <div className="rounded-2xl bg-slate-900/90 border border-slate-800 p-4 space-y-2">
        ...
        <button onClick={() => exportCsv("all")} className="w-full py-2.5 rounded-xl bg-slate-800/80 text-slate-200 text-sm font-medium flex items-center justify-center gap-2 active:scale-95">
          <Download size={14} /> All trips (CSV)
        </button>
        <button onClick={() => exportCsv("business")} className="w-full py-2.5 rounded-xl bg-slate-800/80 text-slate-200 text-sm font-medium flex items-center justify-center gap-2 active:scale-95">
          <Download size={14} /> Business trips only (CSV)
        </button>
        <button onClick={() => exportCsv("chargeable")} className="w-full py-2.5 rounded-xl bg-slate-800/80 border border-sky-500/30 text-sky-400 text-sm font-medium flex items-center justify-center gap-2 active:scale-95">
          <Receipt size={14} /> Chargeable only, for client billing (CSV)
        </button>
        <button onClick={exportSiteVisitsCsv} className="w-full py-2.5 rounded-xl bg-slate-800/80 border border-amber-500/30 text-amber-400 text-sm font-medium flex items-center justify-center gap-2 active:scale-95">
...
function StatCard({ label, value, sub, accent }) {
  const color =
    ...
    "text-slate-100";
  return (
    <div className="rounded-2xl bg-slate-900/90 border border-slate-800 p-3">
    ...
    <div className="space-y-4">
      <div className="rounded-2xl bg-slate-900/90 border border-slate-800 p-4">
        ...
        <div className="flex flex-col gap-2 mb-3">
          {locations.map((loc) => {
            ...
            return (
              <div key={loc.name} className="flex items-center gap-2 px-3 py-2 rounded-xl bg-slate-800/80 border border-slate-700">
                <MapPin size={14} className={hasCoords ? "text-emerald-400 shrink-0" : "text-slate-500 shrink-0"} />
                <span className="flex-1 text-sm text-slate-200">{loc.name}</span>
                <button
                  onClick={() => handlePin(loc.name)}
                  disabled={pinningName === loc.name}
                  className={`text-xs font-medium px-2 py-1 rounded-lg ${hasCoords ? "text-slate-300 bg-slate-700" : "text-amber-400 bg-slate-900 border border-amber-500/30"}`}
                >
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 bg-slate-800/80 border border-slate-700 rounded-xl px-3 py-2 text-sm text-slate-100 placeholder-slate-500 outline-none focus:border-amber-400/50"
          />
          <button
      ...
      <div className="rounded-2xl bg-slate-900/90 border border-slate-800 p-4">
        ...
        <div className="text-xs text-slate-500 mb-3">
          ...
          flow — point it at whatever endpoint your Node-RED instance exposes.
        </div>
        <Field label="Webhook URL">
          <input
            ...
            className="w-full bg-slate-800/80 border border-slate-700 rounded-xl px-3 py-2 text-sm text-slate-100 placeholder-slate-500 outline-none focus:border-sky-400/50 font-mono"
            inputMode="url"
            autoCapitalize="none"
            autoCorrect="off"
          />
        ...
        <div className="grid grid-cols-2 gap-2 mt-1">
          <button
            ...
            className="py-2.5 rounded-xl bg-slate-800/80 border border-slate-700 text-slate-200 text-sm font-medium flex items-center justify-center gap-2 active:scale-95 disabled:opacity-40"
          >
            <Send size={13} />
          ...
          <button
            ...
            className="py-2.5 rounded-xl bg-slate-800/80 border border-sky-500/30 text-sky-400 text-sm font-medium flex items-center justify-center gap-2 active:scale-95 disabled:opacity-40"
          >
            <Radio size={13} />
        ...
        {(pingStatus === "fail" || syncStatus === "fail") && (
          <div className="text-rose-400 text-xs flex items-center gap-1.5 mt-2">
            <AlertTriangle size={13} /> Couldn't reach that URL check address and CORS headers on Node-RED.
          </div>
        )}
      </div>
      ...
      <div className="rounded-2xl bg-slate-900/90 border border-slate-800 p-4">
        ...
        {typeof window !== "undefined" && window.appSignOut && (
          <button
            ...
            className="w-full py-2.5 rounded-xl bg-slate-800/80 border border-slate-700 text-slate-300 text-sm font-medium"
          >
            Sign out
          </button>
        )}
...
function BottomNav({ tab, setTab }) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-slate-950/80 backdrop-blur-md border-t border-slate-800/80 px-2 pb-safe z-20">
...
function Modal({ title, onClose, children, footer }) {
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/80 backdrop-blur-xs" onClick={onClose}>
      <div
        className="w-full max-w-md rounded-t-3xl border-t border-slate-700 bg-slate-950 relative overflow-hidden"
      ...
      >
        <div className="relative z-10 flex flex-col" style={{ maxHeight: "88vh" }}>
          ...
          {footer && <div className="px-5 pb-6 pt-2 border-t border-slate-800 bg-slate-950">{footer}</div>}
        </div>
      </div>
    </div>
  );
...
const inputCls = "w-full bg-slate-800/80 border border-slate-700 rounded-xl px-3 py-2.5 text-slate-100 outline-none focus:border-amber-400/50 font-odo";
const inputClsPlain = "w-full bg-slate-800/80 border border-slate-700 rounded-xl px-3 py-2.5 text-slate-100 outline-none focus:border-amber-400/50";
    ...
    <div className="flex flex-wrap gap-2">
      {locations.map((loc) => (
        <button
          ...
          className={`px-3 py-1.5 rounded-full text-sm font-medium border ${value === loc.name ? "bg-amber-400 text-slate-950 border-amber-400" : "bg-slate-800/80 text-slate-300 border-slate-700"}`}
        >
          {loc.name}
        </button>
      ))}
      <button onClick={() => onToggleCustom(true)} className="px-3 py-1.5 rounded-full text-sm font-medium border border-dashed border-slate-600 text-slate-400 bg-slate-900">
        + Other
      </button>
    </div>
  );
  ...
  async function locate() {
    ...
    try {
      ...
      if (match) {
        ...
        setNote("New place type a name below and it will be remembered.");
        onNoMatch({ lat, lng });
      }
      setStatus("idle");
    } catch (e) {
  ...
  return (
    <div className="mb-2">
      <button
        ...
        className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-slate-800/80 border border-slate-700 text-slate-300 text-xs font-medium disabled:opacity-60"
      >
    ...
    <div>
      <div className="flex gap-2">
        <button
          ...
          className={`flex-1 py-2.5 rounded-xl border text-sm font-semibold flex items-center justify-center gap-1.5 ${value === "business" ? "bg-slate-900 border-emerald-500 text-emerald-400" : "bg-slate-800/80 border-slate-700 text-slate-400"}`}
        >
          <Briefcase size={14} /> Business
        </button>
        <button
          onClick={() => onChange("private")}
          className={`flex-1 py-2.5 rounded-xl border text-sm font-semibold flex items-center justify-center gap-1.5 ${value === "private" ? "bg-slate-900 border-rose-500 text-rose-400" : "bg-slate-800/80 border-slate-700 text-slate-400"}`}
        >
        ...
        <div className="mt-2.5 pl-3 border-l-2 border-slate-800">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className={`flex-1 py-2 rounded-lg border text-xs font-semibold flex items-center justify-center gap-1 ${businessType === "chargeable" ? "bg-slate-900 border-sky-500 text-sky-400" : "bg-slate-800 border-slate-700 text-slate-500"}`}
            >
...
function StartTripModal({ locations, suggestedMileage, onClose, onSave }) {
  ...
  function submit() {
    if (!fromLocation.trim()) return setError("Pick or type where leaving from.");
    if (!mileageOut || Number.isNaN(Number(mileageOut))) return setError("Enter odometer reading.");
    setError("");
    onSave({
...
function EndTripModal({ trip, locations, onClose, onSave }) {
  ...
  function submit() {
    if (!toLocation.trim()) return setError("Pick or type where arrived.");
    if (!mileageIn || Number.isNaN(Number(mileageIn))) return setError("Enter odometer reading.");
    if (Number(mileageIn) < trip.mileageOut) return setError(`Less than odometer out (${fmtKm(trip.mileageOut)}).`);
    setError("");
  ...
  return (
    ...
    >
      <div className="rounded-xl bg-slate-900 border border-slate-800 p-3 mb-3 text-sm">
      ...
      <div className="mb-1 pl-3 border-l-2 border-slate-800">
        <div className="text-xs text-slate-500 mb-1.5">If this is a job site (optional)</div>
        ...
        <Field label="What doing here"><input value={siteNotes} onChange={(e) => setSiteNotes(e.target.value)} placeholder="e.g. Replace NVR" className={inputClsPlain} /></Field>
      </div>
      {error && <div className="text-rose-400 text-xs flex items-center gap-1.5 mb-1"><AlertTriangle size={13} /> {error}</div>}
    </Modal>
  );
...
function FullTripModal({ locations, initial, onClose, onSave, onDelete }) {
  ...
  function submit() {
    ...
    if (Number(mileageIn) <= Number(mileageOut)) return setError("Odometer in must be greater than out.");
    setError("");
    onSave({
  ...
  return (
    <Modal
      title={initial ? "Edit Trip" : "Log Completed Trip"}
      onClose={onClose}
      footer={
        <div>
          {km !== null && (
          ...
          {onDelete && (
            <button onClick={onDelete} className="w-full py-2.5 rounded-xl bg-slate-900 border border-rose-500/40 text-rose-400 font-semibold text-sm flex items-center justify-center gap-2">
    ...
    >
      ...
      <div className="mb-1 pl-3 border-l-2 border-slate-800">
        ...
        <Field label="Job number"><input value={jobNumber} onChange={(e) => setJobNumber(e.target.value)} placeholder="e.g. 4521" className={inputClsPlain} /></Field>
        <Field label="What did there"><input value={siteNotes} onChange={(e) => setSiteNotes(e.target.value)} placeholder="e.g. Replace NVR" className={inputClsPlain} /></Field>
      </div>
      {error && <div className="text-rose-400 text-xs flex items-center gap-1.5 mb-1"><AlertTriangle size={13} /> {error}</div>}
    </Modal>
  );
...
function ConfirmDialog({ title, message, onCancel, onConfirm }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 px-6" onClick={onCancel}>
      <div className="w-full max-w-sm bg-slate-900 border border-slate-700 rounded-2xl p-5" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex gap-2">
          ...
          <button onClick={onConfirm} className="flex-1 py-2.5 rounded-xl bg-rose-600 text-white font-semibold text-sm">Delete</button>
        </div>
      </div>
    </div>
  );

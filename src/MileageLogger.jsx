...
export default function MileageLogger() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col relative" style={{ fontFamily: "'Manrope', sans-serif" }}>
      ...
      {appBgImage && (
        <>
          <img src={appBgImage} alt="" className="fixed inset-0 w-full h-full object-cover z-0" />
  ...
  return (
    <div className="px-5 pt-6 pb-5 border-b border-slate-800/80">
      <div className="flex items-center justify-between mb-3">
        ...
        {activeTrip && (
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-slate-900 border border-emerald-500">
...
function LogTab({ activeTrip, recentTrips, onStart, onEnd, onFull, onViewAll, onEditTrip }) {
  return (
    <div className="space-y-4">
      {activeTrip ? (
        <div className="rounded-2xl bg-slate-900 border border-emerald-500 p-4">
        ...
        <button
          onClick={onStart}
          className="w-full py-5 rounded-2xl bg-amber-400 text-slate-950 font-bold text-base flex items-center justify-center gap-2 active:scale-95 transition-transform"
        >
...
function TripRow({ trip, onClick, compact }) {
  ...
  const iconWrapCls = isChargeable ? "bg-slate-800 text-sky-400" : isBiz ? "bg-slate-800 text-emerald-400" : "bg-slate-800 text-rose-400";
  return (
    <button
      onClick={onClick}
      className="w-full text-left rounded-xl bg-slate-900 border border-slate-800 p-3 flex items-center gap-3 transition-colors"
    >
...
function SummaryTab({ trips }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {unspecifiedCount > 0 && (
        <div className="text-xs text-amber-400 bg-slate-900 border border-amber-500/30 rounded-xl px-3 py-2">
      ...
      <div className="rounded-2xl bg-slate-900 border border-slate-800 p-4 space-y-2">
        ...
        <button onClick={() => exportCsv("chargeable")} className="w-full py-2.5 rounded-xl bg-slate-800 border border-sky-500/30 text-sky-400 text-sm font-medium flex items-center justify-center gap-2 active:scale-95">
          <Receipt size={14} /> Chargeable only, for client billing (CSV)
        </button>
        <button onClick={exportSiteVisitsCsv} className="w-full py-2.5 rounded-xl bg-slate-800 border border-amber-500/30 text-amber-400 text-sm font-medium flex items-center justify-center gap-2 active:scale-95">
      ...
      <div className="rounded-2xl bg-slate-900 border border-slate-800 p-4">
        ...
        <div className="grid grid-cols-2 gap-2 mt-1">
          ...
          <button
            ...
            className="py-2.5 rounded-xl bg-slate-800 border border-sky-500/30 text-sky-400 text-sm font-medium flex items-center justify-center gap-2 active:scale-95 disabled:opacity-40"
          >
            <Radio size={13} />
...
function BottomNav({ tab, setTab }) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-slate-950 border-t border-slate-800 px-2 pb-safe z-20">
    ...
    <div>
      <div className="flex gap-2">
        <button
          ...
          className={`flex-1 py-2.5 rounded-xl border text-sm font-semibold flex items-center justify-center gap-1.5 ${value === "business" ? "bg-slate-900 border-emerald-500 text-emerald-400" : "bg-slate-800 border-slate-700 text-slate-400"}`}
        >
          <Briefcase size={14} /> Business
        </button>
        <button
          onClick={() => onChange("private")}
          className={`flex-1 py-2.5 rounded-xl border text-sm font-semibold flex items-center justify-center gap-1.5 ${value === "private" ? "bg-slate-900 border-rose-500 text-rose-400" : "bg-slate-800 border-slate-700 text-slate-400"}`}
        >

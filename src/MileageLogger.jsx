...
function SummaryTab({ trips }) {
  ...
  function exportCsv(filter) {
    const rows = trips
      ...
      .sort((a, b) => (sortKey(a) > sortKey(b) ? 1 : -1));
    const header = ["Date", "Time Out", "From", "Odometer Out", "Time In", "To", "Odometer In", "KM", "Category", "Business Type", "Client", "Purpose"];
...
function EndTripModal({ trip, locations, onClose, onSave }) {
  ...
  function submit() {
    ...
    setError("");
    onSave({ timeIn, mileageIn, toLocation: toLocation.trim(), toLocationCoords: toCustom ? toCoords : null });
  }
...
function FullTripModal({ locations, initial, onClose, onSave, onDelete }) {
  ...
  function submit() {
    ...
    onSave({
      ...
      toLocation: toLocation.trim(), category, businessType, client, purpose,
      fromLocationCoords: fromCustom ? fromCoords : null,

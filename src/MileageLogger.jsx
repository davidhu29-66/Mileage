...
function LogTab({ activeTrip, recentTrips, onStart, onEnd, onFull, onViewAll, onEditTrip }) {
  return (
    <div className="space-y-4">
      {activeTrip ? (
        <div className="rounded-2xl bg-slate-900 border border-emerald-500/30 p-4">
...
function TripRow({ trip, onClick, compact }) {
  ...
  return (
    <button
      onClick={onClick}
      className="w-full text-left rounded-xl bg-slate-800 hover:bg-slate-750 border border-slate-700/80 p-3 flex items-center gap-3 transition-colors"
    >
...
function SummaryTab({ trips }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {unspecifiedCount > 0 && (
        <div className="text-xs text-amber-400 bg-slate-900 border border-amber-400/40 rounded-xl px-3 py-2">
...
function BottomNav({ tab, setTab }) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-slate-950 border-t border-slate-800 px-2 pb-safe">
...
function Modal({ title, onClose, children, footer }) {
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/90" onClick={onClose}>
      <div
        className="w-full max-w-md bg-slate-950 rounded-t-3xl border-t border-slate-800 relative overflow-hidden"
    ...
    <div>
      <div className="flex gap-2">
        <button
          ...
          className={`flex-1 py-2.5 rounded-xl border text-sm font-semibold flex items-center justify-center gap-1.5 ${value === "business" ? "bg-slate-900 border-emerald-400 text-emerald-400" : "bg-slate-800 border-slate-700 text-slate-400"}`}
        >
          <Briefcase size={14} /> Business
        </button>
        <button
          onClick={() => onChange("private")}
          className={`flex-1 py-2.5 rounded-xl border text-sm font-semibold flex items-center justify-center gap-1.5 ${value === "private" ? "bg-slate-900 border-rose-400 text-rose-400" : "bg-slate-800 border-slate-700 text-slate-400"}`}
        >
        ...
        <div className="mt-2.5 pl-3 border-l-2 border-slate-800">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className={`flex-1 py-2 rounded-lg border text-xs font-semibold flex items-center justify-center gap-1 ${businessType === "chargeable" ? "bg-slate-900 border-sky-400 text-sky-400" : "bg-slate-800 border-slate-700 text-slate-500"}`}
            >
    ...
    <Modal
      ...
      footer={
        <div>
          ...
          {onDelete && (
            <button onClick={onDelete} className="w-full py-2.5 rounded-xl bg-slate-900 border border-rose-400/40 text-rose-400 font-semibold text-sm flex items-center justify-center gap-2">
...
function ConfirmDialog({ title, message, onCancel, onConfirm }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/90 px-6" onClick={onCancel}>

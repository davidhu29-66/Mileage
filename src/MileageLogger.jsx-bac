    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col relative" style={{ fontFamily: "'Manrope', sans-serif" }}>
      ...
      {appBgImage && (
        <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none">
          <img src={appBgImage} alt="" className="w-full h-full object-cover object-center" />
...
function Header({ lastMileage, lastMileageMeta, activeTrip }) {
  return (
    <div className="px-5 pt-6 pb-5 border-b border-slate-800/60 bg-slate-950/40 backdrop-blur-md">
      <div className="flex items-center justify-between mb-3">
        ...
        {activeTrip && (
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-500/15 border border-emerald-400/30 backdrop-blur-sm">
...
function LogTab({ activeTrip, recentTrips, onStart, onEnd, onFull, onViewAll, onEditTrip }) {
  return (
    <div className="space-y-4">
      {activeTrip ? (
        <div className="rounded-2xl bg-slate-900/70 backdrop-blur-md border border-emerald-400/30 p-4">
      ...
      <button
        onClick={onFull}
        className="w-full py-3 rounded-xl bg-slate-900/60 backdrop-blur-md border border-slate-700/60 text-slate-300 font-semibold text-sm flex items-center justify-center gap-2 active:scale-95 transition-transform"
      >
        <Plus size={16} /> Log a completed trip
      </button>

      <div className="rounded-2xl bg-slate-900/60 backdrop-blur-md border border-slate-800/80 p-4">
        <div className="flex items-center justify-between mb-3">
        ...
        {recentTrips.length === 0 ? (
          <div className="text-center py-6">
            <div className="text-slate-400 text-sm">No trips logged yet</div>
            <div className="text-slate-500 text-xs mt-1">Tap Start Trip when you head out</div>
          </div>
        ) : (
...
function TripRow({ trip, onClick, compact }) {
  ...
  const iconWrapCls = isChargeable ? "bg-sky-400/20 text-sky-400" : isBiz ? "bg-emerald-400/20 text-emerald-400" : "bg-rose-400/20 text-rose-400";
  return (
    <button
      onClick={onClick}
      className="w-full text-left rounded-xl bg-slate-900/50 backdrop-blur-sm hover:bg-slate-800/70 border border-slate-700/50 p-3 flex items-center gap-3 transition-colors"
    >
  ...
  if (trips.length === 0) {
    return (
      <div className="text-center py-16">
        <List size={28} className="text-slate-600 mx-auto mb-3" />
        <div className="text-slate-400 text-sm font-medium">No trips yet</div>
        <div className="text-slate-500 text-xs mt-1">Your logged trips will show up here</div>
      </div>
    );
  }
  const groups = {};
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <button onClick={() => setMonthOffset((m) => m - 1)} className="w-9 h-9 rounded-full bg-slate-900/60 backdrop-blur-md border border-slate-700 flex items-center justify-center active:scale-95">
        ...
        <button
          ...
          className="w-9 h-9 rounded-full bg-slate-900/60 backdrop-blur-md border border-slate-700 flex items-center justify-center active:scale-95 disabled:opacity-30"
        >
      ...
      {unspecifiedCount > 0 && (
        <div className="text-xs text-amber-300 bg-amber-400/10 backdrop-blur-md border border-amber-400/30 rounded-xl px-3 py-2">
      ...
      {clientRows.length > 0 && (
        <div className="rounded-2xl bg-slate-900/60 backdrop-blur-md border border-slate-800 p-4">
            ...
            {monthSiteVisits.map((v, i) => (
              <div key={i} className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  ...
                  {v.notes && <div className="text-xs text-slate-400 truncate">{v.notes}</div>}
                </div>
      ...
      <div className="rounded-2xl bg-slate-900/60 backdrop-blur-md border border-slate-800 p-4 space-y-2">
        <div className="text-sm font-semibold text-slate-300 mb-1">Export for tax / reimbursement</div>
        <button onClick={() => exportCsv("all")} className="w-full py-2.5 rounded-xl bg-slate-800/80 backdrop-blur-sm text-slate-200 text-sm font-medium flex items-center justify-center gap-2 active:scale-95">
          <Download size={14} /> All trips (CSV)
        </button>
        <button onClick={() => exportCsv("business")} className="w-full py-2.5 rounded-xl bg-slate-800/80 backdrop-blur-sm text-slate-200 text-sm font-medium flex items-center justify-center gap-2 active:scale-95">
          <Download size={14} /> Business trips only (CSV)
        </button>
        <button onClick={() => exportCsv("chargeable")} className="w-full py-2.5 rounded-xl bg-sky-400/10 backdrop-blur-sm border border-sky-400/30 text-sky-400 text-sm font-medium flex items-center justify-center gap-2 active:scale-95">
          <Receipt size={14} /> Chargeable only, for client billing (CSV)
        </button>
        <button onClick={exportSiteVisitsCsv} className="w-full py-2.5 rounded-xl bg-amber-400/10 backdrop-blur-sm border border-amber-400/30 text-amber-400 text-sm font-medium flex items-center justify-center gap-2 active:scale-95">
...
function StatCard({ label, value, sub, accent }) {
  const color =
    ...
    "text-slate-100";
  return (
    <div className="rounded-2xl bg-slate-900/60 backdrop-blur-md border border-slate-800 p-3">
      ...
      <div className="rounded-2xl bg-slate-900/60 backdrop-blur-md border border-slate-800 p-4">
        ...
        <div className="flex flex-col gap-2 mb-3">
          {locations.map((loc) => {
            ...
            return (
              <div key={loc.name} className="flex items-center gap-2 px-3 py-2 rounded-xl bg-slate-800/60 border border-slate-700/60">
                ...
                <button
                  ...
                  className={`text-xs font-medium px-2 py-1 rounded-lg ${hasCoords ? "text-slate-300 bg-slate-700/60" : "text-amber-400 bg-amber-400/10 border border-amber-400/30"}`}
                >
                  {pinningName === loc.name ? "Pinning…" : hasCoords ? "Re-pin" : "Pin here"}
                </button>
                <button onClick={() => onRemoveLocation(loc.name)}><X size={13} className="text-slate-400" /></button>
              </div>
            );
          })}
        </div>
...
function BottomNav({ tab, setTab }) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-slate-950/80 backdrop-blur-lg border-t border-slate-800/80 px-2 pb-safe z-20">
...
function Modal({ title, onClose, children, footer, bgImage }) {
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 backdrop-blur-xs" onClick={onClose}>
      <div
        className="w-full max-w-md rounded-t-3xl border-t border-slate-700/80 relative overflow-hidden bg-slate-900/90 backdrop-blur-xl"
      ...
      >
        ...
        <div className="relative z-10 flex flex-col" style={{ maxHeight: "88vh" }}>
          ...
          <div className="flex items-center justify-between px-5 pt-3 pb-2">
            ...
            <button onClick={onClose} className="w-8 h-8 rounded-full bg-slate-800/80 flex items-center justify-center">
              <X size={15} className="text-slate-400" />
            </button>
          </div>
          <div className="px-5 pb-3 overflow-y-auto no-scrollbar">{children}</div>
          {footer && <div className="px-5 pb-6 pt-2 border-t border-slate-800/80">{footer}</div>}
        </div>
      </div>
    </div>
  );
...
function LocationChips({ locations, value, onChange, customMode, onToggleCustom }) {
  ...
  return (
    <div className="flex flex-wrap gap-2">
      ...
      <button onClick={() => onToggleCustom(true)} className="px-3 py-1.5 rounded-full text-sm font-medium border border-dashed border-slate-600 text-slate-400 bg-slate-900/50">
        + Other
      </button>
    </div>
  );
...
function LocateButton({ locations, onMatch, onNoMatch }) {
  ...
  return (
    <div className="mb-2">
      ...
      {note && (
        <div className={`text-xs mt-1.5 ${status === "error" ? "text-rose-400" : "text-slate-400"}`}>{note}</div>
      )}
    </div>
  );
}
    ...
    <div>
      <div className="flex gap-2">
        <button
          ...
          className={`flex-1 py-2.5 rounded-xl border text-sm font-semibold flex items-center justify-center gap-1.5 ${value === "business" ? "bg-emerald-400/15 border-emerald-400 text-emerald-400" : "bg-slate-800/80 border-slate-700 text-slate-400"}`}
        >
          <Briefcase size={14} /> Business
        </button>
        <button
          onClick={() => onChange("private")}
          className={`flex-1 py-2.5 rounded-xl border text-sm font-semibold flex items-center justify-center gap-1.5 ${value === "private" ? "bg-rose-400/15 border-rose-400 text-rose-400" : "bg-slate-800/80 border-slate-700 text-slate-400"}`}
        >
      ...
      {value === "business" && (
        <div className="mt-2.5 pl-3 border-l-2 border-slate-800">
          <div className="text-xs text-slate-400 mb-1.5">Business trip type</div>
          <div className="flex gap-2">
            <button
              onClick={() => onBusinessTypeChange("admin")}
              className={`flex-1 py-2 rounded-lg border text-xs font-semibold ${businessType !== "chargeable" ? "bg-slate-700 border-slate-500 text-slate-100" : "bg-slate-800/80 border-slate-700 text-slate-400"}`}
            >
              Admin
            </button>
            <button
              onClick={() => onBusinessTypeChange("chargeable")}
              className={`flex-1 py-2 rounded-lg border text-xs font-semibold flex items-center justify-center gap-1 ${businessType === "chargeable" ? "bg-sky-400/15 border-sky-400 text-sky-400" : "bg-slate-800/80 border-slate-700 text-slate-400"}`}
            >
...
function EndTripModal({ trip, locations, onClose, onSave }) {
  ...
  return (
        ...
        <div>
          {km !== null && km >= 0 && (
            <div className="text-center mb-3">
              ...
              <span className="text-slate-400 text-sm ml-1">km travelled</span>
            </div>
          )}
          <button onClick={submit} className="w-full py-3.5 rounded-xl bg-amber-400 text-slate-950 font-bold flex items-center justify-center gap-2">
    ...
    >
      <div className="rounded-xl bg-slate-800/50 p-3 mb-3 text-sm border border-slate-700/50">
      ...
      <div className="mb-1 pl-3 border-l-2 border-slate-800">
        <div className="text-xs text-slate-400 mb-1.5">If this is a job site (optional — you can also fill this in later from History)</div>
...
function FullTripModal({ locations, initial, onClose, onSave, onDelete }) {
  ...
  return (
    <Modal
      ...
      footer={
        <div>
          ...
          {onDelete && (
            <button onClick={onDelete} className="w-full py-2.5 rounded-xl bg-rose-400/15 border border-rose-400/30 text-rose-400 font-semibold text-sm flex items-center justify-center gap-2">
    ...
    >
      ...
      <Field label="Purpose (optional)"><input value={purpose} onChange={(e) => setPurpose(e.target.value)} placeholder="e.g. Site inspection" className={inputClsPlain} /></Field>
      <div className="mb-1 pl-3 border-l-2 border-slate-800">
        <div className="text-xs text-slate-400 mb-1.5">Job site details for "{toLocation || "To"}" (optional)</div>
        <Field label="Job number"><input value={jobNumber} onChange={(e) => setJobNumber(e.target.value)} placeholder="e.g. 4521" className={inputClsPlain} /></Field>
...
function Toast({ type, message }) {
  return (
    <div className="fixed bottom-24 left-1/2 -translate-x-1/2 z-50 px-4 py-2.5 rounded-full bg-slate-800/90 border border-slate-700 shadow-xl flex items-center gap-2 backdrop-blur-md">
